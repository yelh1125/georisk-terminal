...
export default function Dashboard() {
  ...
  const radarData = latest ? [
    ['布伦特冲击', latest.brentZ], ['相关性转移', latest.correlationZ], ['波动率', latest.vixZ], ['流动性压力', latest.liquidityZ], ['期权情绪', latest.sentimentZ],
    ...
    <main className="min-h-screen bg-canvas px-4 py-6 text-ink sm:px-6 lg:px-10">
      <div className="mx-auto max-w-7xl">
        <header className="flex flex-col gap-5 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-teal">GeoRisk Terminal</p><h1 className="mt-1 text-2xl font-semibold">地缘政治风险实时监测</h1><p className="mt-1 text-sm text-slate-500">收盘五因子模型 · GDELT 事件脉冲 · 最近收盘 {formatDate(latest.date)}</p></div>
        ...
        {nowcast ? <section className="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]" aria-label="实时风险脉冲">
          <div className="border border-teal/30 bg-white shadow-panel">
            <div className="grid gap-0 lg:grid-cols-[1.35fr_repeat(4,minmax(0,1fr))]">
              ...
              <div className="border-b border-slate-200 p-5 lg:border-b-0 lg:border-r"><p className="text-xs font-medium uppercase tracking-[0.1em] text-slate-500">市场最快因子日期</p><strong className="mt-2 block text-lg">{nowcast.marketAsOf}</strong><p className="mt-2 text-xs leading-5 text-slate-500">Brent {nowcast.factorAsOf.brent} · VIX {nowcast.factorAsOf.vix}<br />相关 {nowcast.factorAsOf.correlation} · 信用 {nowcast.factorAsOf.liquidity} · SKEW {nowcast.factorAsOf.sentiment}</p></div>
              <div className="p-5"><p className="text-xs font-medium uppercase tracking-[0.1em] text-slate-500">布伦特收盘 / GPR 发布</p><strong className="mt-2 block text-lg">${nowcast.brentClose.toFixed(2)} · {nowcast.gprAsOf}</strong><p className="mt-2 text-xs leading-5 text-slate-500">Brent 是模型因子；AI-GPR 仅作回测基准</p></div>
            </div>
          </div>
          <aside className="border border-slate-200 bg-white p-5 shadow-panel" aria-label="实时风险指标说明">
            ...
            <dl className="mt-4 space-y-2.5 text-xs leading-5 text-slate-600">
              <div><dt className="font-semibold text-ink">六因子实时得分</dt><dd>0.20 布伦特原油五日冲击 Z + 0.30 新闻脉冲 Z + 0.20 股债相关 Z + 0.15 VIX Z + 0.10 BAA10Y Z + 0.05 CBOE SKEW Z。AI-GPR 权重为 0%。</dd></div>
