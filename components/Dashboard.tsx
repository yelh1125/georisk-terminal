...
export default function Dashboard() {
  ...
  const radarData = latest ? [
    ['Brent-WTI 价差', latest.oilSpreadZ], ['原油 IV (OVX)', latest.oilIvZ], ['Gold/Oil', latest.goldOilZ], ['市场传导', latest.marketTransmissionZ],
        ...
        {nowcast ? <section className="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]" aria-label="实时风险脉冲">
          <div className="border border-teal/30 bg-white shadow-panel">
            <div className="grid gap-0 lg:grid-cols-[1.35fr_repeat(4,minmax(0,1fr))]">
              ...
              <div className="p-5"><p className="text-xs font-medium uppercase tracking-[0.1em] text-slate-500">布伦特最新日线</p><strong className="mt-2 block text-lg">${nowcast.brentClose.toFixed(2)} · {nowcast.brentAsOf}</strong><p className="mt-2 text-xs leading-5 text-slate-500">数据：{nowcast.brentSource === 'twelve_data_xbr_usd' ? 'Twelve Data XBR/USD（Brent Spot）' : nowcast.brentSource === 'yahoo_bz_f' ? 'Yahoo Finance BZ=F（前月期货日线）' : 'FRED DCOILBRENTEU（现货日度，可能滞后）'}<br />用于 Brent-WTI 价差因子</p></div>
            </div>
          </div>
